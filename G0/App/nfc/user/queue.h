...
//缓冲队列的个数需要为2的幂
#define QUEUE_NODE_NUM        (16)         //缓冲队列的大小（有多少个缓冲区）
#define QUEUE_NODE_DATA_LEN   (32)       //单个接收缓冲区大小
...
{
	char  *head; 	//缓冲区头指针	
	volatile uint16_t len; //接收到的数据长度
...
void pull_data_from_queue2(uint8_t *data,uint8_t len);																								 
