...
{ 
extern bool linkokState;
	if(linkokState)
	{ 
		static uint32_t lastTick,currentTick;
		extern uint8_t get_card(void);
    
		char cache[64];
	//	currentTick = HAL_GetTick();
		memset(cache,0,sizeof(cache));
    extern int car_number;
		uint16_t len = sprintf(cache,"%d/%d",car_number,100);
		SendCOMdata(USER_SourceID,cache,len,0xED);	
...
{ 
	...
	{		
     if(rx_data->head[0] == 0x5A && rx_data->head[rx_data->len - 1] == 0xA5)
		 { 
