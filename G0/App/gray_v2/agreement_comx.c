...
#include "senords.h"   /* USER_ObjectID/USER_SourceID 由产品宏 GRAY_V2=1 决定 */
...
void frame_parser_init(FrameParser *parser) {
    ...
    if(parser->buffer != NULL)
		{
		   sys_free(parser->buffer);
		}
		parser->buffer = sys_malloc(MAX_FRAME_SIZE);
															 ...
															 uint8_t index)
{
    ...
    frame.length = len+7;
	  ...
	  frame.data[0] = 0x5A;
	  frame.data[1] = USER_ObjectID;
	  frame.data[2] = USER_SourceID;
	  frame.data[3] = len;
	  frame.data[4] = index;
